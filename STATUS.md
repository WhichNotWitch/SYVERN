...
|---|---|---|---|
| L0 Pilot parse/resolve/typecheck | `PilotAdapter` against local Pilot HTTP service at `http://127.0.0.1:8888` | `services/pilot-server` real backend via `RealPilotBackend`, Java 21, `jupyter-sysml-kernel-0.59.0-all.jar`, and `sysml.library` | `/health`, `/version`, and `/validate` verified locally; `compilePilotRealJava` verified with the SysML kernel jar; CLI exposes only `--adapter pilot`; fingerprint composition includes the Pilot endpoint |
...
| Configuration | dataclass defaults with local Pilot endpoint | `SYVERN_...` environment variables via `load_settings_from_env`; local Pilot launch settings in ignored `scripts/pilot-real.local.ps1` | env parsing tests for endpoints, storage, auth, thresholds, and weights; `scripts/start-pilot-real.ps1` supports `GRADLE_EXE`, Java 21 toolchain, and isolated `.gradle-user-home` |
...
| CI | GitHub Actions | compileall + ruff + mypy + pytest | workflow and tool config present; local tests verified; real Pilot alignment is intentionally operator-run because it depends on a local service and SysML kernel installation |

## Alignment Datasets

`data/alignment/pilot_real_corpus.jsonl` is the current real-Pilot alignment input. The `syvern align` CLI can enforce overall, parse, resolve, typecheck, minimum case count, and required category thresholds against the configured Pilot endpoint.

`data/alignment/stub_smoke.jsonl` remains an internal deterministic fixture for hermetic tests and historical comparison; it is not the public default validation path.

The phase2 acceptance target remains a manually annotated SysML v2 dataset with at least 50 cases for real Pilot/MontiCore outputs, covering syntax errors, unresolved references, type errors, valid models, and nested/scale gradients.

## Still Open

- Calibrate and run the real-Pilot alignment suite against the local Pilot service on the annotated corpus.
- Add the full >= 50 case annotated SysML v2 alignment dataset.
- Decide the production Pilot endpoint/deployment shape beyond the current local `127.0.0.1:8888` service.
