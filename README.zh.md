...
环境变量、后端选择、安全/RBAC、监控、测试和本机 Pilot Server。
...
```
                ┌──────────────────────────────────────────────┐
   模型文本  →  │  L0  Pilot 参考实现 (权威主判决)               │ ← 主判决       [local HTTP]
                │  L0' MontiCore 独立第二解析器                  │ ← 交叉一致性   [stub/live optional]
...
SYVERN 默认期望真实 Pilot HTTP 服务运行在 `http://127.0.0.1:8888`。当前 SysML v2 Jupyter
kernel jar 需要 Java 21。

```powershell
Copy-Item .\scripts\pilot-real.local.example.ps1 .\scripts\pilot-real.local.ps1
# 编辑 scripts\pilot-real.local.ps1,设置 $JAR、$LIB,以及可选的 $GRADLE_EXE。
powershell -ExecutionPolicy Bypass -File .\scripts\start-pilot-real.ps1
```

然后在另一个终端启动 SYVERN API:
...
pipeline、schema、奖励映射、抗作弊与监控面已全部实现,并由 **306 个通过的测试**验证。里程碑 H1–H6
...
**下一步:** 用本机 Pilot 服务校准真实 Pilot 对齐语料,确定 `127.0.0.1:8888` 之外的生产 Pilot endpoint/部署形态,在部署中配置 `cache_path` / `record_store_path` / 保留上限,并在真实后端延迟目标明确后设置 hosted SLA 阈值。
